...
## AI (PPO2  알고리즘)과 위험성회피 전략(LPPL모델, Turblence index)을 적용시킨 강화학습 트레이더
  ...
  - 특이 사항: 현 모델은 향후 다른 프로젝트에 사용될 예정 이므로 코드는 요청시 공개.(wjtls01@naver.com , 010-4402-5325)
...
## 결론
   ...
   - ## 검증 데이터 셋 결과<br/>
     ...
     - ## 이외 추가적 방법
     - ![image](https://user-images.githubusercontent.com/60399060/147627146-dddb2093-726a-4262-914e-fd0736f8b105.png)
     - 위 그래프: (validation data set에서) AI의 Portfolio value , 아래 그래프: SP500 주가
     - 방법: 어떤 X데이터와 Y데이터를 학습 시킨 에이전트에게 Z데이터,Y데이터를 보여준다
     - 시장 수익률 : 8.411694 %
     - AI 에이전트 수익률 : 28.9773 %
     - MDD(Maximum draw down : 최대 손실폭) : 시장MDD= 약 -35% 일때 에이전트는 최대 -11.393 % 의 낙폭
     - 결론: 이외의 추가적 방법을 사용할 경우 에이전트는 하락장과 상승장 모두 강한 결과를 보인다.  <br/><br/>
